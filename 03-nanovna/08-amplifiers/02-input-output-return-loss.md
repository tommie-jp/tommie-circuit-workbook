...
- C2 (出力結合コンデンサ、0.1 µF) は測る周波数 (1〜300 MHz) では
  リアクタンスが 1 Ω 台と小さく、あってもなくても出力側の見え方はほとんど
  変わらない。実体配線図では省いて Rc の節点をそのまま J2 へつなぐ

## 実体配線図

8-1 と同じ考え方の perfboard (アッテネータは付けない。C2 も上の理由で省く)。
...
- C2 (出力結合コンデンサ) は 8-1 にはあったが、ここではコレクタの生の
  インピーダンスを見たいので**外し**、Rc の節点をそのまま J2 へつなぐ
